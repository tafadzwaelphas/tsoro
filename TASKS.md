...
20. **Fixed the tutorial callout position** (2026-07-12, two rounds of user feedback after item 19 shipped): the board's rows are only 75px apart with a 30px pit radius, leaving just a 15px gap between rows. The callout's first placement (44px above the target pit's center) sat almost exactly on that gap boundary, so it visually read as pointing at the pit in the row *above* the actual target — user caught this from a screenshot. Moved the marker to rest inside the target pit's own circle instead of floating in the inter-row gap, and tightened the bounce animation to match. A follow-up request ("move it down a couple pixels, bottom of the circle or middle at most") repositioned it from the upper half to the lower half of the pit. Net effect: the 👆 now sits unambiguously inside the lower half of the glowing target pit. Verified visually via Playwright screenshot zoomed on the board after each change — this is a case where the SVG-coordinate math looked reasonable on paper but only rendering + a real screenshot caught the ambiguity.
...
- README.md and QUICKVIEW.md now point at the GitHub Pages URL too (both previously still referenced the abandoned Netlify link).
